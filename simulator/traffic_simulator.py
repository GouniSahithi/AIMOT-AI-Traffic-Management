...
timeElapsed = 0
ambulance_override_handled = False
skipNextLane = None
lastScheduledGreen = 0
resumeAfterAmbulance = False
resume_from_lane = 0
next_after_ambulance = None
event_log = []


signal_order = deque([0, 1, 2, 3])
currentGreen = signal_order[0]
ambulance_handled = False
ambulance_queue_backup = deque()
served_ambulance_lanes = set()
ambulance_priority_used = False
skip_lane_once = None
...
class Vehicle(pygame.sprite.Sprite):
    def __init__(self, lane, vehicleClass, direction_number, direction, will_turn):
        ...
        self.speed = speeds.get(vehicleClass.strip(), 2.0)
        ...
        try:
            path = "traffic_images/images/" + direction + "/" + vehicleClass + ".png"
            self.originalImage = pygame.image.load(path)
            self.currentImage = self.originalImage
        except Exception as e:
            print(f"⚠️ Could not load image for {vehicleClass} in {direction}: {e}")
            self.originalImage = pygame.Surface((40, 40))
...
def setTime():
    ...
    print("Detecting vehicles in", directionNumbers[(currentGreen+1)%noOfSignals])
...
def repeat():
    global signal_order, currentGreen, currentYellow
    global ambulance_priority_used, next_after_ambulance, skip_lane_once

    # 🚨 Ambulance override (only once)
    if not ambulance_priority_used and ambulance_detected[2]:
        current_index = signal_order.index(currentGreen)
        next_after_ambulance = signal_order[(current_index + 1) % len(signal_order)]

        if 2 in signal_order:
            signal_order.remove(2)
        signal_order.appendleft(2)

        print("🚨 Ambulance detected in lane 3 (LEFT). Prioritizing it.")
        event_log.append(f"t={timeElapsed}: ✅ Signal priority given to lane 3")

        ambulance_detected[2] = False
        ambulance_priority_used = True
    ...
    while signals[currentGreen].green > 0:
        printStatus()
        updateValues()
        if signals[signal_order[1] % 4].red == detectionTime:
    ...
    # ✅ Restore normal queue and mark lane 2 to skip once
    if ambulance_priority_used and currentGreen == 2:
        print("✅ Ambulance lane completed. Resuming correct rotation.")
        signal_order = deque([0, 1, 2, 3])
        while signal_order[0] != next_after_ambulance:
            signal_order.rotate(-1)
        ambulance_priority_used = False
        skip_lane_once = 2  # lane 2 should be skipped once

    else:
        signal_order.rotate(-1)

    # ⏭️ Skip lane 2 once
    if skip_lane_once is not None and signal_order[0] == skip_lane_once:
        print(f"⏭️ Skipping lane {skip_lane_once} once after ambulance.")
        signal_order.rotate(-1)
        skip_lane_once = None
...
def generateVehicles():
    ...
    global ambulance_spawned
    ambulance_spawned = False

    while True:
        # 🚑 Spawn ambulance from left after 10 seconds
        if not ambulance_spawned and timeElapsed >= 10:
            direction_number = 2  # 'left'
            lane_number = 2      # middle lane
            ...
            print("🚑 Ambulance spawned in lane 4 (LEFT → RIGHT) after 10 sec")
            event_log.append(f"t={timeElapsed}: 🚨 Ambulance detected in lane {direction_number + 1}")

            ambulance_detected[2] = True
            ambulance_spawned = True
            
            continue

        # Random vehicle generation logic
        vehicle_type = random.randint(0, 4)  # avoid ambulance again
        ...
        print(f"Spawned {vehicleTypes[vehicle_type]} in {directionNumbers[direction_number]}")
        time.sleep(0.75)
...
def export_simulation_data():
    global timeElapsed, event_log
    print("[🟢] export_simulation_data() started")
    ...
    while True:
        try:
            ...
            for dir_key in vehicles:
                for lane in range(3):
                    for vehicle in vehicles[dir_key][lane]:
                        vtype = vehicle.vehicleClass
                ...
                "lanes": [
                    {
                        ...
                        "passed": vehicles[directionNumbers[i]]['crossed'],
                        "waiting": sum(len(vehicles[directionNumbers[i]][j]) for j in range(3)),
                        "emergency": ambulance_detected[i]
                    }
                    for i in range(noOfSignals)
                ],
            ...
            if timeElapsed >= simTime:
                output["events"].append(f"✅ Simulation Complete at t={timeElapsed}s")

            with open("simulation_data.json", "w") as f:
                json.dump(output, f, indent=2)

        except Exception as e:
            print(f"[❌] export_simulation_data error: {e}")
...
def Main():
    ...
    background = pygame.image.load('traffic_images/images/mod_int.png')

    screen = pygame.display.set_mode(screenSize)
    pygame.display.set_caption("SIMULATION")

    # Loading signal images and font
    redSignal = pygame.image.load('traffic_images/images/signals/red.png')
    yellowSignal = pygame.image.load('traffic_images/images/signals/yellow.png')
    greenSignal = pygame.image.load('traffic_images/images/signals/green.png')
...
# ✅ Run simulation + export thread
if __name__ == "__main__":
    threading.Thread(target=export_simulation_data, daemon=True).start()
    Main()
